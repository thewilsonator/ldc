...
static int showInstrProfile(const std::string &Filename, bool ShowCounts,
  ...
  std::vector<std::pair<std::string, uint64_t>> HottestFuncs;
  ...
  for (const auto &Func : *Reader) {
    ...
    if (TopN) {
      ...
      if (HottestFuncs.size() == TopN) {
        if (HottestFuncs.front().second < FuncMax) {
          pop_heap(HottestFuncs.begin(), HottestFuncs.end(), MinCmp);
          HottestFuncs.pop_back();
          HottestFuncs.emplace_back(
              std::make_pair(std::string(Func.Name), FuncMax));
          push_heap(HottestFuncs.begin(), HottestFuncs.end(), MinCmp);
        }
      } else
        HottestFuncs.emplace_back(
            std::make_pair(std::string(Func.Name), FuncMax));
  ...
  if (TopN) {
    std::sort(HottestFuncs.begin(), HottestFuncs.end(), MinCmp);

    OS << "Top " << TopN
       << " functions with the largest internal block counts: \n";
    for (auto hotfunc : HottestFuncs)
